...
public class TweakTransformerHelper {
   ...
   public static void quenchFireTick(World world, int x, int y, int z, Random rand) {
      ...
      boolean supported = base != null && base.isFireSource(world, x, y - 1, z, world.getBlockMetadata(x, y - 1, z), ForgeDirection.UP);
      if(!Block.fire.canPlaceBlockAt(world, x, y, z)) {
         ...
         if(!supported && !Block.fire.canBlockCatchFire(world, x, y, z, ForgeDirection.UP) && meta == 15 && rand.nextInt(4) == 0) {
            world.setBlock(x, y, z, 0);
         }
      }
