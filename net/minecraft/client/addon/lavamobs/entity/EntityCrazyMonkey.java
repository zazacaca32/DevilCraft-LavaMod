...
public class EntityCrazyMonkey extends LavaEntityMob
{
    public int smashCounter = 0;
    private boolean smash = false;
    private float jumpValue = 0.15F;
    private int x = 0;
    private int z = 0;
    private int ticktarget = 0;

    public EntityCrazyMonkey(World par1World)
    {
        super(par1World);
        super.texture = "/mods/lavamobs/textures/monkey.png";
        ...
        if (!super.worldObj.isRemote)
        {
            if (super.isSwingInProgressq)
            {
                ...
                if ((-1.0D <= super.motionX && super.motionX <= 1.0D || -1.0D <= super.motionZ && super.motionZ <= 1.0D) && super.onGround && !super.worldObj.isRemote)
                {
                    ...
                    {
                        this.jumpValue = 0.7F + super.rand.nextFloat() / 6.0F;
                    }
                }
            }
            else
    ...
    protected void onDeathUpdate()
    {
        if (!super.worldObj.isRemote)
        {
            this.entityDropItem(new ItemStack(LavaModMobs.TotemItem.itemID, 2, 2), 1.0F);
        }
