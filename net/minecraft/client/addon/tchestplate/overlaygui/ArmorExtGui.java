...
public class ArmorExtGui extends InventoryEffectRenderer
{
    ...
    public ArmorExtGui(InventoryPlayer inventoryplayer, ArmorExtInventory holder)
    {
        super(new ArmorExtContainer(inventoryplayer, holder));
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j)
    {
        ...
        super.mc.renderEngine.bindTexture("/mods/LavaChestPlate/textures/gui/armorextended.png");
        int cornerX = super.guiLeft;
        int cornerY = super.guiTop;
        this.drawTexturedModalRect(cornerX, cornerY, 0, 0, super.xSize, super.ySize);

        if (!this.holderInventory.isStackInSlot(0)) {
        	drawTexturedModalRect(cornerX + 84, cornerY + 7, 64, 166, 16, 16);
          }
        
        if (!this.holderInventory.isStackInSlot(2))
        {
            this.drawTexturedModalRect(cornerX + 132, cornerY + 7, 16, 166, 16, 16);
        }

        if (!this.holderInventory.isStackInSlot(3))
        {
            this.drawTexturedModalRect(cornerX + 108, cornerY + 27, 0, 166, 16, 16);
        }

        if (!this.holderInventory.isStackInSlot(5))
        {
            this.drawTexturedModalRect(cornerX + 132, cornerY + 54, 32, 166, 16, 16);
        }

        if (!this.holderInventory.isStackInSlot(4))
        {
            this.drawTexturedModalRect(cornerX + 84, cornerY + 54, 48, 166, 16, 16);
        }
        if (!this.holderInventory.isStackInSlot(6)) {
        	drawTexturedModalRect(cornerX + 132 + 9, cornerY + 54 - 24, 80, 166, 16, 16);
          }
