...
public class MESlotFakeUP64DonateSpecial extends MESlotBase {
   ...
   private static final LAItemStack doanteItem = LAItemStack.create(new ItemStack(LavaChestPlate.ItemDonate, 1, 1));
   private MESlotFakeUP64DonateSpecial sl1;


   public void setSl1(MESlotFakeUP64DonateSpecial sl1) {
      this.sl1 = sl1;
   }

   public MESlotFakeUP64DonateSpecial getSl1() {
      return this.sl1;
   }

   public MESlotFakeUP64DonateSpecial(ILANetworkInventory cellinvfake, IDonateUser idu, int idx, int x, int y) {
      super((IInventory)null, idx, x, y);
      this.cellinvfake = cellinvfake;
      this.invSlot = idx;
   }

   public void addToInvDonateItem(int count) {
      LAItemStack current = doanteItem.copy();
      current.setStackSize((long)count);
      if(current.getStackSize() == 0L) {
         current = null;
      }
   ...
   public boolean isCoin() {
      return this.getHasStack() && this.getStack().isItemEqual(doanteItem.getItemStack());
   }

   public void incCoin() {
      this.addToInv(doanteItem.getItemStack());
   }
