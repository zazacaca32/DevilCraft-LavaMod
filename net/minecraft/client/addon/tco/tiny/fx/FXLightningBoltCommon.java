...
public class FXLightningBoltCommon {
   ...
   public void fractal(int splits, float amount, float splitchance, float splitlength, float splitangle) {
      if(!this.finalized) {
         ...
         while(true) {
            ...
            WRVector3 splitrot;
            for(i = 1; i < splits; ++i) {
               WRVector3 var18 = WRVector3.getPerpendicular(segment.diff).rotate(this.rand.nextFloat() * 360.0F, segment.diff);
               var18.scale((this.rand.nextFloat() - 0.5F) * amount);
               splitrot = startpoint.copy().add(subsegment.copy().scale((float)i));
               newpoints[i] = new FXLightningBoltCommon.BoltPoint(splitrot, var18);
            }

            for(i = 0; i < splits; ++i) {
               FXLightningBoltCommon.Segment var181 = new FXLightningBoltCommon.Segment(newpoints[i], newpoints[i + 1], segment.light, segment.segmentno * splits + i, segment.splitno);
               if((var181.prev = prev) != null) {
                  prev.next = var181;
               }

               if(i != 0 && this.rand.nextFloat() < splitchance) {
                  splitrot = WRVector3.xCrossProduct(var181.diff).rotate(this.rand.nextFloat() * 360.0F, var181.diff);
                  WRVector3 diff = var181.diff.copy().rotate((this.rand.nextFloat() * 0.66F + 0.33F) * splitangle, splitrot).scale(splitlength);
                  ++this.numsplits;
                  this.splitparents.put(Integer.valueOf(this.numsplits), Integer.valueOf(var181.splitno));
                  FXLightningBoltCommon.Segment split = new FXLightningBoltCommon.Segment(newpoints[i], new FXLightningBoltCommon.BoltPoint(newpoints[i + 1].basepoint, newpoints[i + 1].offsetvec.copy().add(diff)), segment.light / 2.0F, var181.segmentno, this.numsplits);
                  split.prev = prev;
                  this.segments.add(split);
               }

               prev = var181;
               this.segments.add(var181);
            }
   ...
   private void calculateCollisionAndDiffs() {
      ...
      for(iterator = this.segments.iterator(); iterator.hasNext(); lastactiveseg = segment.segmentno) {
      ...
      for(iterator = this.segments.iterator(); iterator.hasNext(); segment.calcEndDiffs()) {
         segment = (FXLightningBoltCommon.Segment)iterator.next();
   ...
   public class SegmentSorter implements Comparator {
      ...
      public int compare(FXLightningBoltCommon.Segment o1, FXLightningBoltCommon.Segment o2) {
         ...
         if(comp == 0) {
            int n = Integer.valueOf(o1.segmentno).compareTo(Integer.valueOf(o2.segmentno));
            return n;
         } else {
            return comp;
         }
