...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float par1, boolean par2, int par3, int par4)
    {
        ...
        if (!this.mc.playerController.enableEverythingIsScrewedUpMode())
        {
            ...
            k1 = foodstats.getFoodLevel();
            l1 = foodstats.getPrevFoodLevel();
            ...
            if (this.mc.playerController.shouldDrawHUD())
            {
                ...
                {
                    short short1 = 182;
                    k2 = (int)(this.mc.thePlayer.experience * (float)(short1 + 1));
                    l2 = l - 32 + 3;
                    this.drawTexturedModalRect(i2, l2, 0, 64, short1, 5);

                    if (k2 > 0)
                    {
                        this.drawTexturedModalRect(i2, l2, 0, 69, k2, 5);
                    }
                }

                k3 = l - 39;
                k2 = k3 - 10;
                l2 = this.mc.thePlayer.getTotalArmorValue();
                i3 = -1;
                ...
                for (j4 = 0; j4 < 10; ++j4)
                {
                    if (l2 > 0)
                    {
                        ...
                        if (j4 * 2 + 1 < l2)
                        {
                            this.drawTexturedModalRect(j3, k2, 34, 9, 9, 9);
                        }

                        if (j4 * 2 + 1 == l2)
                        {
                            this.drawTexturedModalRect(j3, k2, 25, 9, 9, 9);
                        }

                        if (j4 * 2 + 1 > l2)
                        {
                            this.drawTexturedModalRect(j3, k2, 16, 9, 9, 9);
                        }
                    }
                    ...
                    this.drawTexturedModalRect(k4, l4, 16 + b0 * 9, 9 * b1, 9, 9);

                    if (flag1)
                    {
                        if (j4 * 2 + 1 < j1)
                        {
                            this.drawTexturedModalRect(k4, l4, j3 + 54, 9 * b1, 9, 9);
                        }

                        if (j4 * 2 + 1 == j1)
                        {
                            this.drawTexturedModalRect(k4, l4, j3 + 63, 9 * b1, 9, 9);
                        }
                    }

                    if (j4 * 2 + 1 < i1)
                    {
                        this.drawTexturedModalRect(k4, l4, j3 + 36, 9 * b1, 9, 9);
                    }

                    if (j4 * 2 + 1 == i1)
                    {
                        this.drawTexturedModalRect(k4, l4, j3 + 45, 9 * b1, 9, 9);
                    }
                }
                ...
                for (j4 = 0; j4 < 10; ++j4)
                {
                    ...
                    if (this.mc.thePlayer.getFoodStats().getSaturationLevel() <= 0.0F && this.updateCounter % (k1 * 3 + 1) == 0)
                    {
                    ...
                    l4 = i4 - j4 * 8 - 9;
                    this.drawTexturedModalRect(l4, j3, 16 + b2 * 9, 27, 9, 9);

                    if (flag2)
                    {
                        if (j4 * 2 + 1 < l1)
                        {
                            this.drawTexturedModalRect(l4, j3, l3 + 54, 27, 9, 9);
                        }

                        if (j4 * 2 + 1 == l1)
                        {
                            this.drawTexturedModalRect(l4, j3, l3 + 63, 27, 9, 9);
                        }
                    }

                    if (j4 * 2 + 1 < k1)
                    {
                        this.drawTexturedModalRect(l4, j3, l3 + 36, 27, 9, 9);
                    }

                    if (j4 * 2 + 1 == k1)
                    {
                        this.drawTexturedModalRect(l4, j3, l3 + 45, 27, 9, 9);
                    }
                }
                ...
                if (this.mc.thePlayer.isInsideOfMaterial(Material.water))
                {
                    ...
                    for (k4 = 0; k4 < j3 + l3; ++k4)
                    {
                        if (k4 < j3)
                        {
                            this.drawTexturedModalRect(i4 - k4 * 8 - 9, k2, 16, 18, 9, 9);
                        }
                        else
                        {
                            this.drawTexturedModalRect(i4 - k4 * 8 - 9, k2, 25, 18, 9, 9);
                        }
                    }
                }
        ...
        if (this.mc.gameSettings.showDebugInfo)
        {
            ...
            k2 = MathHelper.floor_double(this.mc.thePlayer.posY);
            l2 = MathHelper.floor_double(this.mc.thePlayer.posZ);
            ...
            this.drawString(fontrenderer, String.format("z: %.5f (%d) // c: %d (%d)", new Object[] {Double.valueOf(this.mc.thePlayer.posZ), Integer.valueOf(l2), Integer.valueOf(l2 >> 4), Integer.valueOf(l2 & 15)}), 2, 80, 14737632);
            ...
            if (this.mc.theWorld != null && this.mc.theWorld.blockExists(k3, k2, l2))
            {
                Chunk chunk = this.mc.theWorld.getChunkFromBlockCoords(k3, l2);
                this.drawString(fontrenderer, "lc: " + (chunk.getTopFilledSegment() + 15) + " b: " + chunk.getBiomeGenForWorldCoords(k3 & 15, l2 & 15, this.mc.theWorld.getWorldChunkManager()).biomeName + " bl: " + chunk.getSavedLightValue(EnumSkyBlock.Block, k3 & 15, k2, l2 & 15) + " sl: " + chunk.getSavedLightValue(EnumSkyBlock.Sky, k3 & 15, k2, l2 & 15) + " rl: " + chunk.getBlockLightValue(k3 & 15, k2, l2 & 15, 0), 2, 96, 14737632);
            }

            this.drawString(fontrenderer, String.format("ws: %.3f, fs: %.3f, g: %b, fl: %d", new Object[] {Float.valueOf(this.mc.thePlayer.capabilities.getWalkSpeed()), Float.valueOf(this.mc.thePlayer.capabilities.getFlySpeed()), Boolean.valueOf(this.mc.thePlayer.onGround), Integer.valueOf(this.mc.theWorld.getHeightValue(k3, l2))}), 2, 104, 14737632);
            GL11.glPopMatrix();
            this.mc.mcProfiler.endSection();
        }
        ...
        if (this.mc.gameSettings.keyBindPlayerList.pressed && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.playerInfoList.size() > 1 || scoreobjective != null))
        {
            ...
            i2 = (k - k1 * l1) / 2;
            byte b3 = 10;
            drawRect(i2 - 1, b3 - 1, i2 + l1 * k1, b3 + 9 * k5, Integer.MIN_VALUE);

            for (j2 = 0; j2 < j5; ++j2)
            {
                k3 = i2 + j2 % k1 * l1;
                k2 = b3 + j2 / k1 * 9;
                drawRect(k3, k2, k3 + l1 - 1, k2 + 8, 553648127);
                GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
                GL11.glEnable(GL11.GL_ALPHA_TEST);
                ...
                if (j2 < list.size())
                {
                    ...
                    fontrenderer.drawStringWithShadow(s3, k3, k2, 16777215);
                    ...
                    if (scoreobjective != null)
                    {
                        ...
                        if (l3 - j3 > 5)
                        {
                            ...
                            fontrenderer.drawStringWithShadow(s4, l3 - fontrenderer.getStringWidth(s4), k2, 16777215);
                        }
                    }
                    ...
                    this.zLevel += 100.0F;
                    this.drawTexturedModalRect(k3 + l1 - 12, k2, 0 + b4 * 10, 176 + b0 * 8, 10, 8);
                    this.zLevel -= 100.0F;
                }
            }
        }
